...
from adctest.helpers.exceptions import BaseSelectException, UnexpectedTagError, NoSuchElementError
from adctest.pages import WebElementProxy


class Select:
    """
    Класс-хелпер для работы с элементами интерфейса типа select (в ангуляре: ng-select).
    ...
    """
    _options_container_class = 'ng-dropdown-panel'
    ...
    def _find_options(self, by: str, value: str) -> List[WebElement]:
        """
        Ищет все опции ng-select
        :param by:
        :param value:
        :return:
        """
        self.container.click()
        self.container.page_wait.until(EC.visibility_of_element_located((By.TAG_NAME, self._options_container_class)))
        options = self.wait_options_loading(by=by, value=value)

        if len(options) == 1:
            if self._is_empty_option(options[0]):
                return []
    ...
    def select_by_index(self, index: int) -> None:
        """
        Выбирает элемент по его порядковому номеру, начиная с 0
        :param index:
        :return:
        """
        try:
    ...
    def select_by_visible_id(self, obj_id: int, select_all: bool = False) -> None:
        """
        Выбирает опцию по видимому id элемента.
        Пример [12] Offers name
        :param obj_id:
    ...
    def select_by_visible_text(self, text, select_all: bool = False, filter_key: Union[str, int, None] = None) -> None:
        """
        Выбирает option, который будет найден по переданному тексту
        :param text:
        :param select_all: select all found options if it is True and select is multiple
        :param filter_key:
        :return:
        """
        xpath = f'//*[contains(@class,"{self._option_class}") and contains(text(),"{text}")]'
